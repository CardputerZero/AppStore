...
void PackageOperationController::finish(const std::string &output, int result_code)
{
    ...
    if (!ok) {
        ...
        session_.error.title = upper_ascii(job_action_label(action)) + " FAILED";
        ...
        if (pending_conflict) {
            ...
            session_.error.detail = "An earlier " + pending_action + " for " +
                ...
                " is incomplete. Enter Repair to recover and retry it.";
        }
        ...
        session_.status.value() = "Upgraded. Return to launcher to test.";
    } else if (output.find("INSTALLED") != std::string::npos) {
        session_.status.value() = "Installed. Return to launcher to test.";
    } else {
...
void PackageOperationController::poll_backend(uint32_t now)
{
    ...
    if (job_.phase == PackageJobPhase::Repair && rc == 0 &&
        output.find("PACKAGE_REPAIRED\t") != std::string::npos) {
        job_.phase = PackageJobPhase::Prepare;
        job_.pending_start = true;
        job_.clear_backend_result(true);
        job_.detail = "Package transaction repaired. Retrying";
        job_.start_tick = now;
        session_.status.value() = "Package transaction repaired. Retrying...";
        return;
    }
...
void PackageOperationController::poll(uint32_t now, uint32_t worker_start_delay_ms)
{
    ...
    if (started == PackageWorkerStart::Started) {
        session_.status.value() = job_action_label(job_.action) + " " +
            one_line(job_.title, 18) + "... 0s";
    }
    poll_backend(now);
